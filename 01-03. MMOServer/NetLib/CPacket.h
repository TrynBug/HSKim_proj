...
namespace netlib
{
	...
	public:
		...
		/* get */
		int GetBufferSize() const { return _size; }               // 버퍼 전체 크기
		int GetUseSize() const { return _offsetRear; }            // 사용중인 헤더+데이터 크기 (헤더는 항상 있다고 가정함)
		int GetHeaderSize() const { return _sizeHeader; }         // 사용중인 헤더 크기 (헤더는 항상 있다고 가정함)
		int GetDataSize() const { return _offsetRear - _sizeHeader; }  // 사용중인 데이터 크기
		char* GetRearPtr() const { return _buf + _offsetRear; }   // 버퍼의 헤더포함 사용영역 끝 포인터
		char* GetFrontPtr() const { return _buf + _offsetFront; } // 버퍼의 헤더포함 아직 읽지않은영역 시작 포인터
		char* GetDataPtr() const { return _buf + _sizeHeader; }   // 버퍼의 데이터영역 시작 포인터
		char* GetHeaderPtr() const { return _buf; }               // 버퍼의 헤더영역 시작 포인터
		bool IsHeaderSet() const { return _isHeaderSet; }         // 헤더 세팅 여부
		bool IsEncoded() const { return _isEncoded; }             // 암호화 완료 여부
		...
		static void FreePacket(CPacket& packet);
