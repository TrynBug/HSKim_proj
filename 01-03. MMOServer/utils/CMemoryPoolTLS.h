...
private:
	...
	mutable SRWLOCK _srwlVecThreadMgr;
...
public:
	int GetFreeCount() const { return _numFreeChunk * _chunkSize; }
	int GetAllocCount() const { return _numAllocChunk * _chunkSize; }
	int GetActualAllocCount() const;
	int GetPoolSize() const { return (_numFreeChunk + _numAllocChunk) * _chunkSize; }
...
template <typename _Ty>
int CMemoryPoolTLS<_Ty>::GetActualAllocCount() const
{
