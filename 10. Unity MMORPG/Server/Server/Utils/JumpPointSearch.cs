...
namespace Server
{
    ...
    public class JumpPointSearch
    {
        ...
        CellInfo[,] _cells;
        Node _startNode;
        ServerCore.PriorityQueue<Node> _openList = new ServerCore.PriorityQueue<Node>();
        ...
        public void Init(Map map, Cell[,] cells, int maxX, int maxY)
        {
            _map = map;
            _cellsOrigin = cells;
            _maxX = maxX;
        ...
        public List<Vector2> SearchPath(Vector2Int start, Vector2Int end)
        {
            ...
            if (_start == _end)
                return new List<Vector2> { _map.CellToCenterPos(_start), _map.CellToCenterPos(_end) };
            ...
            if(_path.Count == 0)
            {
                smoothPath.Add(_map.CellToCenterPos(_start));
                return smoothPath;
            }
            smoothPath.Add(_map.CellToCenterPos(_path[0]));
            for (int from = 0; from < _path.Count - 1;)
            {
                ...
                for (int to = _path.Count - 1; to > from; to--)
                {
                    ...
                    if (isAccessible)
                    {
                        smoothPath.Add(_map.CellToCenterPos(_path[to]));
                        from = to;
                        break;
                    }
                }
