...
CJsonObject& CJsonObject::Key(const std::wstring& _key) const
{ 
	return g_jsonNone; 
} 

// (virtual) CJsonArray 클래스만 구현함. index 위치의 객체를 리턴함. 기본적으로는 CJsonNone 클래스 객체를 리턴함.
CJsonObject& CJsonObject::Index(int _index) const
{
	return g_jsonNone; 
}
...
CJsonObject& CJsonStruct::Key(const std::wstring& _key) const
{
...
CJsonObject& CJsonArray::Index(int _index) const
{
...
// 토큰 출력
void CJsonParser::PrintTokens() const
{
	wprintf(L"\n");
