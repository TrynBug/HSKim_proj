...
CNetServer::CNetServer()
	...
	, _initialContents(nullptr), _poolMsgContents(0, false, 100)
{
...
void CNetServer::Shutdown()
{
	...
	{
		tick = GetTickCount64();
		retWait = WaitForSingleObject(pContents->_hThreadContents, (DWORD)timeout);
		if (retWait != WAIT_OBJECT_0)
		{
			...
			TerminateThread(pContents->_hThreadContents, 0);
		}
	...
	for (auto& pContents : _listContents)
		CloseHandle(pContents->_hThreadContents);
...
bool CNetServer::SendPacket(__int64 sessionId, CPacket** arrPtrPacket, size_t numPacket)
{
	if (numPacket < 1)
		return false;
...
bool CNetServer::SendPacketAsync(__int64 sessionId, CPacket** arrPtrPacket, size_t numPacket)
{
	if (numPacket < 1)
		return false;
...
// send가 완료된 후 연결을 끊는다.
bool CNetServer::SendPacketAndDisconnect(__int64 sessionId, CPacket** arrPtrPacket, size_t numPacket)
{
	CSession* pSession = FindSession(sessionId);
	if (pSession == nullptr)
		return false;
	...
	if (InterlockedExchange8((char*)&pSession->_bCloseWait, true) == false)
	{
		pSession->_lastPacket = arrPtrPacket[numPacket - 1];
	...
	bool result = SendPacket(sessionId, arrPtrPacket, numPacket);
...
// 비동기 send를 요청하고, send가 완료되면 연결을 끊는다.
bool CNetServer::SendPacketAsyncAndDisconnect(__int64 sessionId, CPacket** arrPtrPacket, size_t numPacket)
{
	CSession* pSession = FindSession(sessionId);
	if (pSession == nullptr)
		return false;
	...
	bool result = SendPacketAsync(sessionId, arrPtrPacket, numPacket);
...
/* 컨텐츠 */
// 세션이 최초 접속시 할당될 컨텐츠를 지정한다.
void CNetServer::SetInitialContents(CContents* pContents)
{
	_initialContents = pContents;
}


// 컨텐츠를 라이브러리에 추가하고 스레드를 시작한다.
bool CNetServer::AttachContents(CContents* pContents)
{
	pContents->_pNetServer = this;
	AcquireSRWLockExclusive(&_srwlListContents);
	_listContents.push_back(pContents);
	ReleaseSRWLockExclusive(&_srwlListContents);
	return pContents->StartUp();
}

// 컨텐츠를 라이브러리에서 제거하고 스레드를 종료한다.
void CNetServer::DetachContents(CContents* pContents)
{
...
__int64 CNetServer::GetContentsInternalMsgCount()
{
	__int64 sum = 0;
	AcquireSRWLockShared(&_srwlListContents);
	for (const auto& pContents : _listContents)
		sum += pContents->GetInternalMsgCount();
	ReleaseSRWLockShared(&_srwlListContents);
	return sum;
}

__int64 CNetServer::GetContentsSessionMsgCount()
{
	__int64 sum = 0;
	AcquireSRWLockShared(&_srwlListContents);
	for (const auto& pContents : _listContents)
		sum += pContents->GetSessionMsgCount();
	ReleaseSRWLockShared(&_srwlListContents);
	return sum;
}

__int64 CNetServer::GetContentsDisconnByHeartBeat()
{
	__int64 sum = 0;
	AcquireSRWLockShared(&_srwlListContents);
	for (const auto& pContents : _listContents)
		sum += pContents->GetDisconnByHeartBeat();
	ReleaseSRWLockShared(&_srwlListContents);
	return sum;
}
...
void CNetServer::UpdateAcceptThread()
{
	while (true)
	{
		...
		MsgContents* pMsg = _poolMsgContents.Alloc();
		pMsg->type = EMsgContentsType::INITIAL_JOIN_SESSION;
		pMsg->pSession = pNewSession;
		pMsg->data = nullptr;
		_initialContents->InsertMessage(pMsg);
		...
		else if (pOverlapped->ioType == IO_RECV)
		{
			...
			else if (numByteTrans > 0)
			{
				...
				while (true)
				{
					...
					pSession->_msgQ->Enqueue(pRecvPacket);
				}
			}
